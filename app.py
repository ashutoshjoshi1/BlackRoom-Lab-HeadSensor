...
DEFAULT_COM_PORTS = {
    ...
    "HEADSENSOR": "COM12"  # Head Sensor / Filter Wheel
}
...
class FilterWheelController:
    """Control filter wheels via Head Sensor using RS-232 serial communication."""
    def __init__(self):
        self.device = SerialDevice(DEFAULT_COM_PORTS["HEADSENSOR"], 4800, 0)
        self.device.eol = "\r"  # Filter wheel expects \r line ending
        # Initialize hst_pars structure based on API
        self.hst_pars = [
            None,      # [0] Port handle (will be set when opened)
            "RS232",   # [1] Connection type
            None,      # [2] (unused)
            "Pan70HST", # [3] Device ID
            None,      # [4] (unused)
            ...
            0,         # [14] FW1 offset
            None, None, None, None, None, None,  # [15-20] (other parameters)
            0          # [21] Auto-search mode (0=auto, 1=manual port)
        ]
        # Initialize serial status structure
        self.serial_status = {
            "hst": [
                ...
                [       # [10] Recovery information
                    ...
                    [0, 0, 0, 0, 0], # [4] Recovery attempt counters per level
    ...
    def configure_port(self, port: str):
    ...
    def _ensure_open(self):
        """Ensure port is open, raise if cannot open."""
        if not self.is_open():
            ok = self.open()
            if not ok:
                raise RuntimeError(f"Could not open Head Sensor serial port '{self.device.port}'")
            self.hst_pars[0] = self.device.ser

    def set_headsensor(self, part: str, pos="r", callfree=0, callinfo=[], stupdate=True):
        """
        ...
        -----------
        part : str
            Component identifier: "F1" for Filter Wheel 1, "F2" for Filter Wheel 2
        pos : int or str
            Position command: Integer 1-9 to move to that position, or "r" to reset
        callfree : int, optional
            Port release behavior: 0=free port after completion (default)
        ...
        if pos == "r":
            ...
            # Position commands: use very short timeout for quick response (hardware is working)
            # Wait only briefly for response, then assume success
            timeout_seconds = 0.3  # Very short timeout for quick UI response
            self.serial_status["hst"][3] = 5.0  # High-level max timeout per API (kept for compatibility)
            self.serial_status["hst"][7] = timeout_seconds  # Low-level timeout
        ...
        try:
            ...
            if response:
                ...
                if len(response_str) >= 3 and response_str[:2] == part:
                    try:
                        ...
                        if error_code == 0:
                            ...
                            if callfree == 0:
                                # Free port after completion
                                pass  # Keep port open for now (can close if needed)
                            return True
                        else:
                ...
                # For position commands, assume success if no response (hardware is working)
                if isinstance(pos, int):
                    # Position command: hardware is working, so assume success immediately
                    self.serial_status["hst"][0] = 0  # Free
                    self.serial_status["hst"][-1] = "OK"
                    return True
                
                # For reset commands, still check for late responses
                if pos == "r":
                    # Quick check once more for reset response
                    time.sleep(0.2)
                    if self.device.ser.in_waiting > 0:
                        chunk = self.device.ser.read(self.device.ser.in_waiting)
                        response += chunk
                        if b"\r\n" in response:
                            ...
                            if len(response_str) >= 3 and response_str[:2] == part:
                                try:
                                    ...
                                    pass
                    # Reset: assume success if hardware is working
                    self.serial_status["hst"][0] = 0
                    self.serial_status["hst"][-1] = "OK"
                    return True
                
                # Should not reach here, but if we do, assume success
                self.serial_status["hst"][0] = 0
                self.serial_status["hst"][-1] = "OK"
                return True
    ...
    def set_filterwheel(self, fw_number: int, position):
        """
        Convenience method to set filter wheel position.
        ...
        """
        part = f"F{fw_number}"
        return self.set_headsensor(part, pos=position)
    
    def reset_filterwheel(self, fw_number: int):
...
class SpectroApp(tk.Tk):
    def __init__(self):
        ...
        self.filterwheel = FilterWheelController()
