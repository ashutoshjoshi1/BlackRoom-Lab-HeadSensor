...
def build(app):
    ...
    def _build_live_tab():
        ...
        app.filterwheel_combo = ttk.Combobox(right, textvariable=app.filterwheel_var, 
                                              values=["Filter Wheel 1", "Filter Wheel 2"],
                                              state="readonly", width=18)
        app.filterwheel_combo.pack(anchor="w", pady=2)
        
        # Frame to hold filter wheel buttons (initially hidden)
        app.filterwheel_frame = ttk.Frame(right)
        app.filterwheel_frame.pack(anchor="w", pady=(6, 0), fill="x")
        
        # Test button
        app.filterwheel_test_btn = ttk.Button(app.filterwheel_frame, text="Test", 
                                              command=lambda: test_filterwheel())
        app.filterwheel_test_btn.pack(anchor="w", pady=2)
        
        # Position buttons (1-9)
        pos_frame = ttk.Frame(app.filterwheel_frame)
        pos_frame.pack(anchor="w", pady=2)
        app.filterwheel_pos_buttons = []
        for i in range(1, 10):
            btn = ttk.Button(pos_frame, text=f"Position {i}", width=10,
                            command=lambda p=i: set_filterwheel_position(p))
            btn.grid(row=(i-1)//3, column=(i-1)%3, padx=2, pady=2, sticky="w")
            app.filterwheel_pos_buttons.append(btn)
        
        # Reset button
        app.filterwheel_reset_btn = ttk.Button(app.filterwheel_frame, text="Reset",
        ...
        # Callback when filter wheel selection changes
        def on_filterwheel_selection(*args):
        ...
        app.filterwheel_var.trace('w', on_filterwheel_selection)

        def test_filterwheel():
        ...
        def set_filterwheel_position(pos: int):
            """Set filter wheel to specified position."""
            try:
                app._update_ports_from_ui()
                selection = app.filterwheel_var.get()
                if not selection:
