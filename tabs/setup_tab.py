...
def build(app):
    ...
    def _build_setup_tab():
        ...
        # HEADSENSOR connection (separate from lasers)
        headsensor_group = ttk.LabelFrame(frame, text="HEADSENSOR Connection")
        headsensor_group.pack(fill="x", padx=6, pady=6)

        ttk.Label(headsensor_group, text="COM Port:").grid(row=0, column=0, sticky="e", padx=4, pady=4)
        app.headsensor_entry = ttk.Entry(headsensor_group, width=12)
        app.headsensor_entry.grid(row=0, column=1, padx=4, pady=4, sticky="w")

        ttk.Button(headsensor_group, text="Test Connect", command=test_headsensor_connect).grid(row=0, column=2, padx=6)
        
        app.headsensor_status = ttk.Label(headsensor_group, text="●", foreground="red")
        app.headsensor_status.grid(row=0, column=3, padx=4)
